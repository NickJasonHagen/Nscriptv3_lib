...
impl  Nscript{
    ...
    fn formatcode(&mut self,code:&str,name:&str)-> String{
        ...
        filedata = Nscript::convertmultilinefunctionstosinge(filedata);
    ...
    fn convertmultilinefunctionstosinge(filedataarg:String) ->String{
        ...
        loop {
            ...
            let original = "(\n".to_string() + &functionscope + "\n)";
            functionscope = Nstring::replace(&functionscope,",\n", ",");
            let replacewith = "(".to_string() + &functionscope +")";
            filedata = Nstring::replace(&filedata,&original,&replacewith);
        }
        return filedata;
    }
        ...
        for mut xline in codevec.clone(){
            for x in 0..xline.len(){
                ...
                if proceed{
                    ...
                    // else if split(&xline[x],"(").len() > 2 && Nstring::instring(&xline[x],").") == true{
                    //     xline[x] = "3".to_string() + &Nstring::trimprefix(&xline[x]);
                    // }
                        ...
                        match xline[0].as_str(){
                            ...
                            _ =>{
                                match self.checkwordtype(&xline[0]){
                                    ...
                                    NscriptWordTypes::Structfn | NscriptWordTypes::Classfunc | NscriptWordTypes::RustFunction | NscriptWordTypes::Function=>{
                                    ...
                                    //         for x in Nstring::split(&Nstring::replace(&xline[0],").",&format!(") {}.",&selfname))," "){
                                    //             if split(&x,"(").len() > 2 {
                                            ...
                                            _ =>{
                                                ...
                                                if xline.len() == 3 {
                                                    if Nstring::instring(&xline[2],").") {
                                                        ...
                                                        newwordvec.push(xline[1].to_string());
                                                        for x in Nstring::split(&Nstring::replace(&xline[2],").",&format!(") {}.",&selfname))," "){
                                                            if split(&x,"(").len() > 2 {
                                                                newwordvec.push(format!("3{}",x));
                                                            }else{
                                                                newwordvec.push(x);
    ...
    fn executepreproccessedline(&mut self,line:&Vec<Box<str>>,formattedblock: &NscriptExecutableCodeBlock,block:&mut NscriptCodeBlock) ->Option<NscriptVar>{
        match line[0].as_ref(){
            ...
            "RET" =>{
                return Some(NscriptVar::new("return"));
            }
            "RV" =>{
            ...
            "RS" =>{
                return Some(NscriptVar::newstring("return", block.staticstrings[Nstring::trimprefix(&line[1]).parse::<usize>().unwrap_or(0)].to_string() ));
            }
            "RP" =>{
            ...
            "BRF" =>{
                ...
                // }
                for x in 1..line.len(){
                    self.execute_cachedfunction(&line[x], block);
                }
            }
            "AA" =>{ // add self
                let mut onvar = self.storage.getvar(&line[1], block);
    ...
    /// optimized loop for setting a stringvec fast!
    ...
    /// optimized loop for setting a string fast!
    ...
    fn setdefiningword(&mut self,word:&str,equalsfrom: NscriptVar,_formattedblock: &NscriptExecutableCodeBlock, block:&mut NscriptCodeBlock){
    ...
    // calls a rustfunction by given word
   ...
   fn execute_preruststructfn(&mut self, funcname:&str,getargs:&str, formattedblock:&NscriptExecutableCodeBlock,block:&mut NscriptCodeBlock) ->NscriptVar{
        ...
        match splitstruct[0]{
            ...
            "nscript" =>{
                match splitstruct[1]{
                    ...
                    "parsefile" =>{
                        let get = self.executeword(&givenargs[0],&formattedblock,block);
                        self.parsefile(&Nstring::replace(&get.stringdata,";","\n"))
                    }
                    "runfunction" =>{
    ...
    fn raw_scopeextract(&mut self,filedata: &str) -> String {
        let mut parsecode = filedata.to_string();
        loop {
            let rawscope = Nstring::stringbetween(&parsecode,"#raw", "#endraw");
            if rawscope == "" {
                break
            }
            else{
