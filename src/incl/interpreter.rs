...
impl  Nscript{
    ...
    fn setbasicfunctions(&mut self){
        ...
        if self.rustfunctionsindex.len() < 2 {
            ...
            self.insertfn("formatbytes", nscriptfn_formatbytes,"(bytesize)  // returns a B,KB,MB,GB,TB string");
...
impl NscriptStorage{
    ...
    pub fn setdefiningword(&mut self,word:&str,equalsfrom: NscriptVar, block:&mut NscriptCodeBlock){
        match self.checkdefiningwordtype(&word){
            ...
            NscriptWordTypes::Reflection =>{
                ...
                self.setdefiningword(&evalword, equalsfrom,block);
            }
            _ =>{
    ...
    pub fn setprop(&mut self,name:&str,prop:&str,var:NscriptVar,block:&mut NscriptCodeBlock){
        let  mut cname:Box<str> = name.trim().into();
        let  mut pname :Box<str> = prop.trim().into();
        if name.contains("*"){
